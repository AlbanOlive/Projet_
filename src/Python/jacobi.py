...
def jacobi(A, b, x0, eps, itmax):
    return jacobi_tridiag(A, b, x0, eps, itmax) if estTridiagonale(A) else jacobi_(A, b, x0, eps, itmax)


def jacobi_tridiag(A, b, x0, eps, itmax):
    it = 0
    ...
    while err > eps and it < itmax:
        ...
        for i in range(1, n-1):
            S = A[i][i-1] * x_old[i-1] + A[i][i+1] * x_old[i+1]
            x[i] = (b[i] - S) / A[i][i]
        i += 1
        S = A[i][i-1] * x_old[i-1]
        x[i] = (b[i] - S) / A[i][i]
